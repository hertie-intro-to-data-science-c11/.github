...
| [**Distribute grades**](https://github.com/hertie-intro-to-data-science-c11/.github/actions/workflows/distribute-grades.yml) | Send what the grading sheet holds: each student's private `grades-<handle>` repo, the registrar export, and an email (needs the `GRAPH_*` secrets). Dry run first. | Run by instructor |
...
`-- <assignment>-<handle>/        one private repo per student (default; see submit_via)
```
